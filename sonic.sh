...
(async () => {
  ...
  for (let i = 0; i < 100; i++) {
    const randomDelay = Math.floor(Math.random() * 16) + 1; // Random delay between 1 and 16 seconds
    await new Promise(resolve => setTimeout(resolve, randomDelay * 1000)); // Convert seconds to milliseconds
